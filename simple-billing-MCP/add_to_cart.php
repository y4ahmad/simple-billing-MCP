...
function exec_ogp_module()
{
	...
	$qry_service = "SELECT DISTINCT service_id, home_cfg_id, mod_cfg_id, service_name, remote_server_id, slot_max_qty, slot_min_qty, price_hourly, price_weekly, price_monthly, price_quarterly, price_year, description, img_url FROM OGP_DB_PREFIXbilling_services WHERE service_id=".$db->realEscapeSingle($service_id);
	...
	*/		
	...
	elseif ($_POST['invoice_duration'] == "weekly")
	{
		$price_slot=$row_service['price_weekly'];
	}
	elseif ($_POST['invoice_duration'] == "monthly")
	{
		$price_slot=$row_service['price_monthly'];
	}
	elseif ($_POST['invoice_duration'] == "quarterly")
	{
		$price_slot=$row_service['price_quarterly'];
	}
	elseif ($_POST['invoice_duration'] == "annually")
	{
		$price_slot=$row_service['price_year'];
	}
	else
	{
		$price_slot=$row_service['price_monthly'];
	...
	*/
	...
	if( isset( $_POST["add_to_cart"] ) )
	{
		...
		if(!isset($_REQUEST['RAM_Allocation'])){$_REQUEST['RAM_Allocation']='';}
		if(!isset($_REQUEST['VIP_Ticket'])){$_REQUEST['VIP_Ticket']='';}
		...
		$_SESSION['CART'][$i] = array( "cart_id" => $i,
									   ...
									   "RAM_Allocation" => $_REQUEST['RAM_Allocation'], 
									   "CPU_Priority" => $_REQUEST['CPU_Priority'], 
									   "VIP_Ticket" => $_REQUEST['VIP_Ticket'], 
									   ...
									   "currency" => $currency,
									   "paid" => 0);
		echo '<meta http-equiv="refresh" content="0;url=?m=simple-billing-MCP&amp;p=cart">';
	}
}
?>
