<?php
function saveOrderToDb($user_id,$service_id,$home_name,$ip,$max_players,$qty,$invoice_duration,$price,$remote_control_password,$ftp_password,$cart_id,$location,$RAM_Allocation,$CPU_Priority,$VIP_Ticket,$home_id = "0",$extended = "0"){
	global $db;
	...
	$fields['RAM_Allocation'] = $RAM_Allocation;
	$fields['CPU_Priority'] = $CPU_Priority;
	$fields['VIP_Ticket'] = $VIP_Ticket;
	$fields['price'] = $price;
...
function exec_ogp_module()
{
	...
	if( isset( $_POST["buy"] ) or isset( $_POST["pay_paypal"] ) or isset( $_POST["pay_paygol"] ) or isset( $_POST["pay_skrill"] ) or isset( $_POST["pay_robokassa"] ) )
	{
		if( isset( $_SESSION['CART'] ) )
		{
			...
			foreach($orders as $order) 
			{
				...
				$RAM_Allocation = $order['RAM_Allocation'];
				$CPU_Priority = $order['CPU_Priority'];
				$VIP_Ticket = $order['VIP_Ticket'];
				$price = $order['price'];
				$remote_control_password = $order['remote_control_password'];
				$ftp_password = $order['ftp_password'];
				//Save order to DB
				saveOrderToDb($user_id,$service_id,$home_name,$ip,$max_players,$qty,$invoice_duration,$price,$remote_control_password,$ftp_password,$cart_id,$location,$RAM_Allocation,$CPU_Priority,$VIP_Ticket);
			}
		...
		{
			echo '<meta http-equiv="refresh" content="0;url=home.php?m=simple-billing-MCP&p=paypal&cart_id='.$cart_id.'" >';
		}
		elseif ( !empty( $cart_id ) and isset( $_POST["pay_paygol"] ) and $settings['paygol'] == "1" )
		{
			echo '<meta http-equiv="refresh" content="0;url=home.php?m=simple-billing-MCP&p=paygol&cart_id='.$cart_id.'" >';
		}
		elseif ( !empty( $cart_id ) and isset( $_POST["pay_skrill"] ) and $settings['skrill'] == "1" )
		{
			echo '<meta http-equiv="refresh" content="0;url=home.php?m=simple-billing-MCP&p=skrill&cart_id='.$cart_id.'" >';
		}
		elseif ( !empty( $cart_id ) and isset( $_POST["pay_robokassa"] ) and $settings['robokassa'] == "1" )
		{
			echo '<meta http-equiv="refresh" content="0;url=home.php?m=simple-billing-MCP&p=robokassa&cart_id='.$cart_id.'" >';
		}
	}
	...
	if( isset( $_POST["extend"] ) or isset( $_POST["extend_and_pay_paypal"] ) or isset( $_POST["extend_and_pay_paygol"] ) or isset( $_POST["extend_and_pay_skrill"] ) or isset( $_POST["extend_and_pay_robokassa"] ) )
	{
		...
		foreach($orders as $order) 
		{
			...
			//Save order to DB
			$order_id = saveOrderToDb($user_id,$service_id,$home_name,$ip,$max_players,$qty,$invoice_duration,$price,$remote_control_password,$ftp_password,$cart_id,$location,$RAM_Allocation,$CPU_Priority,$VIP_Ticket,$home_id,"1");
	...
	if( empty( $carts ) )
	{
		...
		?>		
		<a href="?m=simple-billing-MCP&p=shop" class="btn btn-sm btn-primary"><?php print_lang('back'); ?></a>
		<?php
		return;
	}
	...
	<center>
		<table style="width:95%;text-align:center;" class="center">
			...
			<tr class="tr">
			 ...
			 <td>
				<?php 
				echo "<b>".$order['home_name']."</b> [".get_lang($order['invoice_duration']).", ".$order['max_players']." ".get_lang('slots')."]";
				?>
			 </td>
			 <td>
				<?php 
		...
		<table style="width:95%;text-align:left;" class="center">
			...
			?>
			 ...
			 <td>
			  ...
			  {
			  ?>
			 <form method="post" action="home.php?m=simple-billing-MCP&p=create_servers">
			  <input type="hidden" name="cart_id" value="<?php echo $order['cart_id'];?>">
			  <?php
			 if($order['extended'] == "1")
			 {
			...
			?>
			  ...
			  {
			  ?>
			 <form method="post" action="?m=simple-billing-MCP&p=bill">
			  <input type="hidden" name="cart_id" value="<?php echo $order['cart_id'];?>">
