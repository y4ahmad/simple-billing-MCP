...
{		
	...
	{
		$new_price_weekly = $db->realEscapeSingle($_POST['new_price_weekly']);
		$new_price_monthly = $db->realEscapeSingle($_POST['new_price_monthly']);
		$new_price_quarterly = $db->realEscapeSingle($_POST['new_price_quarterly']);
		...
		$qry_change_url = "UPDATE OGP_DB_PREFIXbilling_services
						   SET price_weekly ='".$new_price_weekly."',
							   price_monthly ='".$new_price_monthly."', 
							   price_quarterly ='".$new_price_quarterly."', 
	...
	if(isset($_POST['mod_cfg_id']) AND isset($_POST['remote_server_id']) AND isset($_POST['slot_max_qty']) AND isset($_POST['price_weekly']) AND isset($_POST['price_monthly']) AND isset($_POST['price_quarterly']) AND isset($_POST['price_year']))
	{
		...
		$price_weekly = $db->realEscapeSingle($_POST['price_weekly']);
		$price_monthly = $db->realEscapeSingle($_POST['price_monthly']);
		$price_quarterly = $db->realEscapeSingle($_POST['price_quarterly']);
		$price_year = $db->realEscapeSingle($_POST['price_year']);
		$RAM_Allocation = $db->realEscapeSingle($_POST['RAM_Allocation']);
		...
		$qry_add_service = "INSERT INTO OGP_DB_PREFIXbilling_services(service_id, home_cfg_id, mod_cfg_id, service_name, remote_server_id, slot_max_qty , slot_min_qty, price_hourly, price_weekly, price_monthly, price_quarterly, price_year, RAM_Allocation, description, img_url, ftp, install_method, manual_url, access_rights) VALUES(NULL, '".$home_cfg_id."', '".$mod_cfg_id."', '".$service_name."', '".$remote_server_id."', '".$slot_max_qty."', '".$slot_min_qty."', '0.00', '".$price_weekly."', '".$price_monthly."', '".$price_quarterly."', '".$price_year."', '".$RAM_Allocation."', '".$description."', '".$img_url."', '".$ftp."', '".$install_method."', '".$manual_url."', '".$access_rights."')";
	...
	else if (isset($_POST['modcfgid']) AND isset($_POST['homecfgid']))
	{
		...
		<tr>
			<td><?php print_lang('price_weekly');?></td>
			<td><input name="price_weekly" type="text" size="8" value="0"/></td>
		</tr>
		<tr>
		...
		<tr>
			<td><?php print_lang('price_quarterly');?></td>
			<td><input name="price_quarterly" type="text" size="8" value="0"/></td>
		</tr>
		<tr>
			<td><?php print_lang('price_year');?></td>
			<td><input name="price_year" type="text" size="8" value="0"/></td>
		</tr>
		<?php if(in_array($mod['mod_cfg_id'], array(1,2,3,4,5,11,12,110,111,112,113,114,131,132,133,134,135,234,235))){ ?>
		<tr>
			<td><?php print_lang('RAM_Allocation');?></td>
			<td>
			...
			<?php 
			  ...
			  while($qty<=10)
			  {
			  echo "<option value='$qty'>$qty GB</option>";
			  $qty++;
			  }
			  ?>
			</select> </td>
	...
	if ($services > 0)
	{
		...
		<tr>
			...
			<th><?php print_lang('price_weekly');?></th>
			<th><?php print_lang('price_monthly');?></th>
			<th><?php print_lang('price_quarterly');?></th>
		...
		<tr class="tr<?php  $i = 0; echo($i++%2);?>">
			...
			<td><input name="new_price_weekly" type="text" value="<?php echo $row['price_weekly'];?>" size="6"/></td>
			<td><input name="new_price_monthly" type="text" value="<?php echo $row['price_monthly'];?>" size="6"/></td>
			<td><input name="new_price_quarterly" type="text" value="<?php echo $row['price_quarterly'];?>" size="6"/></td>
