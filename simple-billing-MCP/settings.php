...
{		
	...
	$settings['Normal'] = isset($settings['Normal']) ? $settings['Normal'] : 1;
	$settings['High'] = isset($settings['High']) ? $settings['High'] : 1;
	$settings['Real_Time'] = isset($settings['Real_Time']) ? $settings['Real_Time'] : 1;
	
	$settings['Yes_VIP_Ticket'] = isset($settings['Yes_VIP_Ticket']) ? $settings['Yes_VIP_Ticket'] : 1;
	$settings['No_VIP_Ticket'] = isset($settings['No_VIP_Ticket']) ? $settings['No_VIP_Ticket'] : 1;
	$settings['available_VIP_Ticket'] = isset($settings['available_VIP_Ticket']) ? $settings['available_VIP_Ticket'] : 1;
    ...
    if ( isset($_REQUEST['update_settings']) )
    {
        $settings = array(
			...
			"High" => @$_REQUEST['High'],
			"Real_Time" => @$_REQUEST['Real_Time'],
			"available_VIP_Ticket" => @$_REQUEST['available_VIP_Ticket'],
			...
			/*
			"Yes_VIP_Ticket" => @$_REQUEST['Yes_VIP_Ticket'],
			"No_VIP_Ticket" => @$_REQUEST['No_VIP_Ticket'],
			*/
        ...
        $view->refresh("?m=simple-billing-MCP&p=shop_settings");
        return;
    }
	...
	$robokassa_Result_URL = $current_folder_url.'modules/simple-billing-MCP/robokassa-ipn.php';
    ...
    $ft = new FormTable();
    $ft->start_form("?m=simple-billing-MCP&p=shop_settings");
    $ft->start_table();
	...
	$ft->add_custom_field('weekly','<input type="checkbox" name="weekly" value="1" '.checked('weekly').'/>');
	$ft->add_custom_field('monthly','<input type="checkbox" name="monthly" value="1" '.checked('monthly').'/>');
	$ft->add_custom_field('quarterly','<input type="checkbox" name="quarterly" value="1" '.checked('quarterly').'/>');
	$ft->add_custom_field('annually','<input type="checkbox" name="annually" value="1" '.checked('annually').'/>');
	
	if($settings['available_VIP_Ticket']==0){$No_VIP_Ticket_checked = "checked";}else{$No_VIP_Ticket_checked = ""; }
	
	echo "<tr><td colspan='2' ><h4>".get_lang('available_CPU_Priority')."</h4></td></tr>";
	$ft->add_custom_field('Normal','<input type="checkbox" name="Normal" value="1" '.checked('Normal').'/>');
	$ft->add_custom_field('High','<input type="checkbox" name="High" value="1" '.checked('High').'/>');
	$ft->add_custom_field('Real_Time','<input type="checkbox" name="Real_Time" value="1" '.checked('Real_Time').'/>');
	
	echo "<tr><td colspan='2' ><h4>".get_lang('available_VIP_Ticket')."</h4></td></tr>";
	$ft->add_custom_field('Yes_VIP_Ticket','<input type="radio" name="available_VIP_Ticket" value="1" '.checked('available_VIP_Ticket').'/>');
	$ft->add_custom_field('No_VIP_Ticket','<input type="radio" name="available_VIP_Ticket" value="0" '.$No_VIP_Ticket_checked.'/>');
