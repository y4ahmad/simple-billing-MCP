	...
	{		
		...
		if( !empty( $orders ) )
		{
			...
			if ($total === 0)
			{
				...
				$view->refresh("home.php?m=simple-billing-MCP&p=cart",0);
			}
			else
			{
				...
				echo '<script src="http://www.paygol.com/micropayment/js/paygol.js" type="text/javascript"></script>'.
					 ...
					 ' <input type="hidden" name="pg_return_url" value="'.urlencode($this_script.'?m=simple-billing-MCP&p=cart').'">'."\n".
					 ' <input type="hidden" name="pg_cancel_url" value="'.$this_script.'?m=simple-billing-MCP&p=cart">'."\n".
					 ' <input type="hidden" name="pg_notify_url" value="'.$current_folder_url.'modules/simple-billing-MCP/paygol-ipn.php">'."\n".
